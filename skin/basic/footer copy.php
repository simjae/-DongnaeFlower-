...
.reviewBtn{
	...
	?>
	<div id="bottom"> 
		<!--<div class="bot_copy wrapper" style="position:fixed;bottom:0px;height:calc(constant(safe-area-inset-bottom) + 110px);height:calc(env(safe-area-inset-bottom) + 110px);z-index:800;font-size:10px;padding-top: 20px;padding-bottom:20px;">-->
		<div class="bot_copy wrapper" style="position:fixed;bottom:0px;height:calc(constant(safe-area-inset-bottom) + 80px);height:calc(env(safe-area-inset-bottom) + 80px);z-index:800;font-size:10px;padding-top: 0px;padding-bottom:20px;">
			<div class="myOrderBtn flex" style="border-bottom: solid 1px #9e9e9e36;justify-content: center;font-size: 1.3em;color: #282828;font-weight: 900;padding-bottom:15px;display:none">
				<font style="float:left;">내 주문정보</font>
				<img style="margin-left: 20px;float:left;" class="orderArrow imgSize" src="/app_beta/skin/basic/svg/productFlower_arrow_up.svg" alt="">
			</div>
...
//						window.location="jscall://appLoginProc";
		...
		function myOrderBtnShowEvent(){
			$('.myOrderBtn').click(function(){
				...
				if ($('.orderStatus').css('display') == "none") {
					...
					$('.bot_copy').css('height','calc(env(safe-area-inset-bottom) + 121px)');
					$('.bot_copy').css('height','calc(constant(safe-area-inset-bottom) + 121px)');
				} else {
					$('.orderArrow').attr('src','/app_beta/skin/basic/svg/productFlower_arrow_under.svg');
					$('.bot_copy').css('height','calc(env(safe-area-inset-bottom) + 464px)');
					$('.bot_copy').css('height','calc(constant(safe-area-inset-bottom) + 464px)');
				}
			});
		}
		...
		function callOrderStatusEvent(member_id,toggleFlg,slideIndex) {
			$.ajax({
				...
				success : function(data) {
								if (data.length > 0) {
									$('.myOrderBtn').show();
									$('.bot_copy').css('padding-top','10px');
									$('.bot_copy').css('height','calc(env(safe-area-inset-bottom) + 121px)');
									$('.bot_copy').css('height','calc(constant(safe-area-inset-bottom) + 121px)');
									$('.bot_copy').css('border-radius','25px 25px 0px 0px');
									$('.orderStatus').css('border-bottom','solid 1px #9e9e9e36');
									orderStatusSplitEvent(data,toggleFlg,slideIndex);
								} else {
									$('.orderStatus').hide();
									$('.bot_copy').css('padding-top','0px');
									$('.bot_copy').css('height','calc(env(safe-area-inset-bottom) + 80px)');
									$('.bot_copy').css('height','calc(constant(safe-area-inset-bottom) + 80px)');
								}
							}
			});
		}
		
		function orderStatusSplitEvent(data,toggleFlg,slideIndex){
			var strDiv = "";
			for(var i = 0; i < data.length; i++){
			strDiv += '<div class="swiper-slide">';
			...
			strDiv += '	<div class="statusIconTable" style="padding:10px 4% 10px 4%;">';
			strDiv += '		<TABLE>';
			strDiv += '			<TR>';
			...
			var strDivPaging = '<div class="orderStatusPagenation" style="margin-bottom: 25px";></div>';
			
			$('.orderStatusWrapper').append(strDiv);
			$('.orderStatusWrapper').after(strDivPaging);
		...
		function setRefreshBtnClickEvent() {
			$('.refreshBtn').click(function() {
				...
				$('.orderStatusWrapper').html('');
				$('.orderStatusPagenation').remove();
