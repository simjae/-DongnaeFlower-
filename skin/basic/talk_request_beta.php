...
<form name="requestForm" id="requestForm" action="request_proc.php" method="post" enctype="multipart/form-data">
	<div id="calendarForm" style="display:none">
		...
		?>
		<div style="width:100%;">
			<input type="text" name="receiveDateInput" maxLength="10" style="width:46%;float:left;margin-right:20px" class="basic_input receiveDate" placeholder="날짜(예 <?=date($todayDate)?>)" value="<?=date($todayDate)?>" readonly/>
			<select name="receiveTimeInput" class="basic_input receiveTime" style="width:46%;float:left;">
	...
	<div id="closeForm" style="display:none">
		...
		?>
		<div>
			<input type="text" name="closeDateInput" maxLength="10" style="width:46%;margin-right:20px; float:left;" class="basic_input closeDate" placeholder="날짜(예 <?=date($todayDate)?>)" value="<?=date($todayDate)?>" readonly/>
			<select name="closeTimeInput" class="basic_input closeTime" style="width:46%;float:left;">
		...
		<table cellpadding="0" cellspacing="0" border="0" style="width:100%">
			<tr>
				<td onclick="stepProc(this);" style="text-align: center;">
					<div style="width:100%">배송지 추가</div>
				</td>
			</tr>
		</table>
	</div>
		...
		<table cellpadding="0" cellspacing="0" border="0" style="width:100%">
			<tr>
				<td style="width:50%">
					...
					<select name="purposeInput" class="basic_input purpose" style="width:100%;">
					<?
				...
				<td style="width:50%;">
					<select name="productTypeInput" class="basic_input productType" style="width:100%;">
					<?
	...
	<div id="priceRangeForm" style="display:none">
		<table cellpadding="0" cellspacing="0" border="0" style="width:100%">
			<tr>
				<td>
					<select name="priceRangeInput" class="basic_input priceRange" style="width:100%;">
					<?
	...
	<div id="styleForm" style="display:none">
		<table cellpadding="0" cellspacing="0" border="0" class="styleForm_select" style="width:100%">
			<tr>
				<td style="text-align: center;" onclick="stepProc(this);">
					<div id="styleSelecter">
						<span>스타일 선택하기</span>
					</div>
				</td>
			</tr>
	...
	<div id="commentForm" style="display:none">
		<textarea name="commentInput" id="commentInput" placeholder="Ex) 가격이 좀 비싸도 오래가는 꽃으로 만들어주세요"></textarea>
	</div>
	...
	<script>
		...
		function sendMessage(editFlag){
			...
			if( messageStr != ""){
				...
				html += "</div>";
				if(!editFlag){
					html += "<div class=\"edit\" onclick=\"editStep('"+ requestStep +"')\">수정</div>";
				}
				html += "</div>";
	...
	<div id="requestFormContent">
		...
		<div class="ui indicating small progress" id="progress01" style="background:none;margin:0px;">
			<div class="bar">
		...
		<script>
			...
			function CheckRequestForm() {
				...
				$.ajax({
					...
					success : function(json){
						if(json["result"] == "Y"){
							location.replace("/app/request_proc.php");
						}
