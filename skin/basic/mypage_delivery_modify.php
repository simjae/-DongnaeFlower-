<?
	...
	if($type=='modify'){
		...
		$onload="<script>alert('배송지 수정이 완료되었습니다.');location.href='mypage_delivery.php';</script>";
	}
...
<style>
	...
	#btn_addr1{width:50%;color: #ed4380;border-radius: 8px;border: solid 1px #ed4380;}
...
<div id="content">
	...
	<div style="width:90%;margin:0 auto;padding:25px 0px;border-bottom:1px solid #9e9e9e36;text-align:center;">
		<h4 style="line-height:20px;font-weight:normal;">상품 주문시 사용하는 배송지 정보를<br />수정할 수 있습니다.</h4>
	</div>
	...
	<div style="width:90%;margin:0 auto;padding:30px 0px;box-sizing:border-box;background:#fff;">
		<form name="form2" action="<?=$_SERVER[PHP_SELF]?>" method="post">
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%" class="delivery_addr">
				...
				<tr>
					<th>배송지명</th>
					<td style="padding:5px 0px;"><input type="text" name="receiver_nick" value="<?=$row->receiver_nick?>" maxlength="20" class="input" style="width:100%;" /></td>
				</tr>
				<tr>
					<th>이름</th>
					<td style="padding:5px 0px;"><input type="text" name="receiver_name" value="<?=$row->receiver_name?>" maxlength="10" class="input" style="width:100%;" /></td>
				</tr>
				<tr>
					<th>연락처</th>
					<td style="padding:5px 0px;">
						<input type="tel" name="receiver_tel1" value="<?=$receiver_tel1?>" maxlength="13" class="input" style="width:100%;" />
					</td>
				</tr>
				<tr>
					<th>주소</th>
					<td style="padding:5px 0px;">
						<div>
							<input type="text" name="rpost1" value="<?=$row->receiver_post?>" maxlength="5" id="rpost1" class="input" style="width:48%;background:#f5f5f5;" onclick="addr_search_for_daumapi('rpost1','raddr1','raddr2')" readOnly />
							<a href="javascript:addr_search_for_daumapi('rpost1','raddr1','raddr2');" id="btn_addr1" class="btn_s_line2">주소검색</a>
						</div>
			...
			<div style="padding-top:20px;border-top:1px solid #9e9e9e36;text-align:center;">
...
<script language="javascript">
	...
	function CheckForm(idx){
		if(document.form2.receiver_name.value.length==0) {
			alert("수신자명을 입력하세요.");
			document.form2.receiver_name.focus();
			return;
		}

		if(!IsNumeric(document.form2.receiver_tel1.value)) {
			alert("전화번호는 숫자만 입력 가능합니다.");
			document.form2.receiver_tel1.focus();
			return;
		}

		if(document.form2.receiver_tel2.value.length==0) {
			alert("휴대폰 번호를 입력하세요.");
			document.form2.receiver_tel2.focus();
			return;
		}
		if(!IsNumeric(document.form2.receiver_tel2.value)) {
			alert("휴대폰 번호는 숫자만 입력 가능합니다.");
			document.form2.receiver_tel2.focus();
			return;
		}

		if(document.form2.rpost1.value.length==0) {
			alert("우편번호를 입력하세요.");
			document.form2.rpost1.focus();
			return;
		}
		if(document.form2.raddr1.value.length==0) {
			alert("주소를 입력하세요.");
			document.form2.raddr1.focus();
			return;
		}
		if(document.form2.raddr2.value.length==0) {
			alert("주소를 입력하세요.");
			document.form2.raddr2.focus();
			return;
		}
		...
		document.form2.type.value="modify";
