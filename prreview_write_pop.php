		...
		if(false !== mysql_query($reviewwriteSQL,get_db_conn())){
			if($_data->review_type=="A") {
				...
				$at_sql = "SELECT
								...
								op.ordercode,
								vi.com_name,
								(SELECT SUM(price) FROM tblorderproduct WHERE ordercode = op.ordercode AND productcode NOT LIKE 'COU%') AS productprice
							FROM
								tblorderproduct op
								LEFT JOIN tblorderinfo oi ON
								op.ordercode = oi.ordercode
				...
				while($at_row = mysql_fetch_object($at_result)) {
					...
					$ordercode = $at_row->ordercode;
					$productprice = number_format($at_row->productprice);
					
					$template_id = "50058";
					$to = array($mobile);
					$vals = array("shopname"=>$com_name,"ordercode"=>$ordercode,"price"=>$productprice);
				...
				$token_sql = "SELECT
								vi.token
							FROM
								tblproductreview prv
								LEFT JOIN tblproduct prd ON
								prv.productcode = prd.productcode
								LEFT JOIN tblvenderinfo vi ON
								prd.vender = vi.vender
							WHERE
								prv.productcode='".$productcode."'";
				$token_result = mysql_query($token_sql,get_db_conn());
				if ($token_row = mysql_fetch_object($token_result)) {
					$token = $token_row->token;
					
					$vender_push_title = "고객님께서 리뷰를 남기셨어요.";
					$vender_push_message = "지금 확인해 보세요.";
					$result = sendPush($vender_push_title, $vender_push_message, $token, '');
				}
			}
		}else{
