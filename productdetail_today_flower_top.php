...
$tel =$_pdata->tel;
$vidx = $_pdata->vender;
$deli_sell_price = number_format($deli_price + $sellprice);
$format_sellprice=number_format($sellprice);
...
<style>
	...
	.typeBtn{border-radius: 30px;color: #282828;font-weight: 500;font-size: 1.0em;border: solid 1px #a0a0a0;padding: 4px 20px;}
...
<div id="detail" style="margin-top:70px;">
<?
	if($mid){ //MD 정보출력
		...
		<div class="orderDetail">
			...
			<div class="flowerWrap">
				<div class="flowerGroup">
					...
					<div class="flex_sb" style="margin-top: 5px;">
						<div class="fn_s2_w3">꽃 이름</div>
						<div class="fn_s2_w3" style="font-weight:900;"><?=$productname?></div>
					</div>
					<div class="flex_sb" style="margin-top: 5px;">
						<div class="fn_s2_w3">판매 가격</div>
						<div><span class="fn_s2_w3" style="text-decoration-line:line-through;margin-right:10px;"></span><span class="fn_s4_w9"><?=number_format($_pdata->sellprice)?>원</span></div>
					</div>
				</div>
			</div>
		
			<div class="order_receiveWrap">
				<div class="order_receiveGroup" style="padding: 20px;">
					<div class="deliPrice">
						<span class="fn_s3_w9">배송 방식</span> 
						<span style="color:#969696; font-weight:500; font-size:1.1em; margin-left:15px;" >*선택 후 배송정보를 확인해 주세요.</span>
					</div>
					<?
						$deli_type_sql = "SELECT deli_type FROM today_flower WHERE productcode='".$productcode."' UNION ALL SELECT deli_type FROM flower_pot WHERE productcode='".$productcode."'";
						$deli_type_result = mysql_query($deli_type_sql,get_db_conn());
						$deli_type_row = mysql_fetch_object($deli_type_result);

						$deli_type = $deli_type_row->deli_type;
						$display_deli = "";
						$onclick_deli = "ReceiverDetailShow('<?=$deliAbleArea?>')";
						$display_pick = "";
						$onclick_pick = "receiveTypeSel(this,'1')";
						
						if ($deli_type == "1") {
							$display_pick = "border:1px solid #c8c8c8;color:#777777;background-color:#ffffff;opacity: 0.4;";
							$onclick_pick = "";
						} else if ($deli_type == "2") {
							$display_deli = "border:1px solid #c8c8c8;color:#777777;background-color:#ffffff;opacity: 0.4;";
							$onclick_deli = "";
						}
					?>
					<div class="receiveBtnType">
						<input type="hidden" value="<?=$deli_type_sql?>">
						<div class="typeBtn" id="deliveryBtn" style="margin-right: 10px;<?=$display_deli?>" onClick="<?=$onclick_deli?>">입력 주소지로 배달</div>
						<div class="typeBtn" id="pickupBtn" style="<?=$display_pick?>" pick= "1" onclick="<?=$onclick_pick?>">해당 꽃집에서 픽업</div>
						<input type="hidden" class="receiveType" name="receiveType" id="detail_receiveType" value=""/>
					</div>
				</div>
			</div>
				
			<div id="addr_group"></div>
			...
			<div id="pick_group">
				<?
					...
					$addr = $row -> com_addr;
				?>
				<div class="addr_item"style="border:none;padding:20px 20px 20px 35px; border-top:solid 1px #9e9e9e36;">
			...
			<div class="pickupReceiveWrap"style="border:none;padding:20px 20px 20px 35px; border-top: 1px solid #9e9e9e36; display:none;">
				...
				<div class="pickupReceiveGroup" style="padding:0">
					<div class= "fn_s3_w9"><span class="receiveText">수령 시간</span><span class="receiveTextNoti" style="color:#969696; font-weight:500; font-size:0.9em; margin-left:15px;" >*직접 받을 수 있는 시간을 선택해 주세요.</span></div>
					<div style="margin-top:10px;">
						<input class="input_selected input start_date" type="text" style="margin-right: 10px;background: url(/app/skin/basic/svg/timesale_product_underArrow.svg) no-repeat 94% 60%/12px;border: solid 1px #a0a0a0;width: 35%;border-radius: 30px;padding: 4px 25px;" size="10"  name="start_date" id="start_date">
			...
			<div class="receivePriceGrop flex_sb" style="margin-top: 0px; border-top:solid 1px #9e9e9e36; display:none;">
				<div>
					<span class="fn_s3_w9">배송비</span>
					<span style="color:#969696; font-weight:500; font-size:1.1em; margin-left:15px;" >*거리, 날씨에 따라 달라질 수 있어요!</span>
				</div> 
				
				<div class="delipriceTxt fn_s4_w9">+<?=number_format($deli_price)?>원</div>
			</div>
			...
			<div class="order_receiveWrap">
				<div class="order_receiveGroup">
					<div class="totalPriceGrop flex_sb">
						...
						<?if($deli_price > 0){
							$totalOrderPrice = $deli_sell_price;
						}else{
							$totalOrderPrice = $format_sellprice;
						}
						?>
						<span id="totalOrderPrice" style="font-size: 1.5em;font-weight: 900;color: #282828;margin-left: 50px;color:#e61e6e"><?=number_format($_pdata->sellprice)?>원</span>
						<!-- <span id="totalOrderPrice" style="font-size: 1.5em;font-weight: 900;color: #282828;margin-left: 50px;color:#e61e6e"><?=$totalOrderPrice?>원</span> -->
						<input type="hidden" id="deli_sell_price" value="<?=$deli_sell_price?>원">
						<input type="hidden" id="sellprice" value="<?=$format_sellprice?>원">
					</div>
				</div>
			</div>
			...
			<!-- //TAB1-기본정보 -->
			<input type="hidden" name="refURL" value="productdetail_today_sale">
			<?if ($refURL == "vender_timesale") {?>
			<input type="hidden" name="refPage" value="vender_timesale">
			<?}?>
			<input type="hidden" name="code" value="<?=$code?>" />
			<input type="hidden" name="productcode" value="<?=$productcode?>" />
			<input type="hidden" name="vidx" value="<?=$_pdata->vender?>">
	...
	*/
	function venderinfoDrop(){
		...
		jQuery(document).ready(function() {
			...
			$("#pick_group").hide();
			$('#start_hour').children('option').click(function(){
				alert('asd');
				$('#start_hour').css('border','solid 1px #464646');
			});
		});
	...
	function CheckForm(gbn,temp2) {
		...
		var pickVal = $('#pickupBtn').attr('pick');
		...
		if(pickVal = 1 ){
			...
			if (current_hour > 20) {
				...
				if (receiveDateTime < currentDateTime1D) {
					alert('수령 시간은 현재 시간보다 세시간 이후에 가능합니다. 올바른 시간을 입력해 주세요.');
					return;
				}
			}
	...
	function receiveTypeSel(obj,val){
		...
		var deli_sell_price = $('#deli_sell_price').val();
		...
		if(val == "1"){
			resetDateCss();
			$("#pick_group").show();
			$('#deliveryReceiveWrap').hide();
			$('.pickupReceiveWrap').show();
			$(".receivePriceGrop").hide();
			$("#addr_group").html("");
			$('#totalOrderPrice').text(sellprice);
			$('.receiveText').text('픽업 시간');
			$('.receiveTextNoti').text('*방문 가능한 시간을 선택해 주세요.');
			$('.start_date').val(todayDay);
		}else{
			resetDateCss();
			$('#deliveryReceiveWrap').show();
			$('.pickupReceiveWrap').show();
			$('.receiveText').text('수령 시간');
			$('.receiveTextNoti').text('*직접 받을 수 있는 시간을 선택해 주세요.');

			$('.start_date').val(todayDay);
			$(".receivePriceGrop").show();
			$("#pick_group").hide();
			$('#totalOrderPrice').text(deli_sell_price);
	...
	//주소록 팝업창
	function ReceiverDetailShow(deliAbleArea){
		$("#delivery_popup").show();
		$("#delivery_content").attr("src","/app/mydeliveryDetailModal.php?deliAbleArea="+deliAbleArea);
	}
	function ReceiverDetailClose(){
		$("#delivery_popup").hide();
		$("#delivery_content").attr("src","about:blank");
	}
	
	function addAddrDetail(name,tel1,tel2,post,addr1,addr2){
		var html = "<div style =\"border-top:solid 1px #9e9e9e36; padding-left:15px\">";
		html += "		<div class=\"addr_item\"style=\"border:none;padding:20px 20px;\">";
		html += "		<div class=\"addrStr_group\" style=\"padding:0\">";
		...
		html += "			<div class=\"addrStr fn_s3_w9\">배달 정보</div>";
		html += "			<div class=\"addrStr fn_s2_w3\"style=\"margin-top: 15px;\">";
		html += addr1;
		html += " ";
		html += addr2;
		html += "			</div>";
		html += "			<div class=\"flex_sb\">";
		html += "				<div class=\"nameTelStr\" style=\"margin-top: 5px;text-align:left;font-size: 1.15em;font-weight: 300;color: #282828;\">";
		html += name + "  (" + tel1 + ")";
		html += "				</div>";
		html += "				<div class=\"changeBtn\" onclick=\"ReceiverDetailShow('<?=$deliAbleArea?>')\">변경</div>";
		html += "			</div>";
		html += "		</div>";
	...
	function pickUpAddr(){
		var html = "<div class=\"addr_item\"style=\"border:none;padding:20px 20px;\">";
		html += "		<div class=\"addrStr_group\" style=\"padding:0\">";
		html += "			<div class=\"addrStr fn_s2_w3\"style=\"margin-top: 15px;\">";
		html +="<?=$addr?>" 
		html += "			</div>";
		html += "		</div>";
		html += "	</div>";
		$("#pick_group").html(html);
		alert($("#pick_group").html(html));
	}
	$(function() {
...
<div id="delivery_popup" style="display: none; position: fixed; padding: 15% 3%; box-sizing: border-box; background: rgba(0, 0, 0, 0.7); z-index: 999; width: 100%; height: 100%; border: 0px solid rgb(221, 221, 221); left: 0%; top: 0%;">
	<div id="btnCloseLayer" style="position:absolute;right:0px;top:0px;left:0px;bottom:0px;z-index:0;" onclick="ReceiverClose()">
		<div style="position:absolute;top:10%;right:3%;color:#fff;font-size:4em;font-weight:500;">×</div>
	</div>
	<div style="position: relative; width: 100%; height: 80%;top:60px; background-color: rgb(255, 255, 255); z-index: 0; overflow: hidden auto; min-width: 300px; margin: 0px; padding: 0px;">
