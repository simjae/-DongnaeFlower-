...
<div id="list">
	<h1 class="list_title">최근 본 상품</h1>
	<div class="wrapper">
		<div class="list_sort">
			<div class="options">
				...
				<li class="product_item">
					...
					<?
						...
						<form name="bfrm<?=$productcode?>" id="bfrm<?=$productcode?>" method="post" action="./basket.php">
							...
							<input type="hidden" name="assemblequantity" />
							<input type="hidden" name="packagenum" />
							<input type="hidden" name="assemble_type" />
							<input type="hidden" name="assemble_list" />
...
<!--모바일 아이콘-->
		...
		<?
					...
					if($i==$gelleryNumRows-1){
						//상품 전체갯수가 3의 배수로 떨어지지 않는다면 
						//비어있는 li를 3의 배수만큼 추가
							if(($i + 1)%3 == 1){
								echo "<li class='product_item'></li><li class='product_item'></li>";
							}
							else if(($i + 1)%3 == 2){
								echo "<li class='product_item'></li>";
							}
...
<!--
	...
	function changeDisplayMode(displaymode,sort){
		location.href="productnew.php?list_type="+displaymode+"&sort="+sort;
		return;
	}
//-->
</script>
