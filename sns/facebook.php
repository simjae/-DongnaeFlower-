...
$facebook->setSession($response);

$id		= $facebook->getSocialId().$userid;
// 네이버 로그인 API 에서 취득한 유저 정보를 솔루션 회원 정보에서 비교.
$sql	= sprintf("select * from tblmember where member_out ='N' AND loginType = 'facebook' AND id = '%s'", $id);
...
if ($facebook->getConnectState() && $cnt) {
	...
	$_SESSION[FB_LOGIN_SESS] = array(
		'userid'	=> $userid,
		'username'	=> $username
	);

	echo "/m/member_join.php?loginType=facebook";
	exit;
}