...
if(false !== $mainPRRes = mysql_query($mainPRSQL,get_db_conn())){
	...
	if($rowcount>0){
		...
		while($mainPRRow = mysql_fetch_assoc($mainPRRes)){
			...
			switch($mainPRRow['title']){
				case "최근 본 상품": //갤러리 타입
?>
					<div class="main_prlist_list">
						<div class="wrapper">
							<div >
								<h1 onclick="location.href='productresent.php'"><img src = "/m/images/skin/default/resent_prod.png" style="max-width: 100%; height: auto; cursor:pointer;"></h1>		
							</div>
							<? if(!empty($origPRList)){ ?>
													...
													<? if(!$dicker['memOpen']){ ?>
														<div class="product_amount">
															...
															<? }else{ ?>
																...
																?>
																	<div><img src = "/m/skin/basic/img/1.png" onClick="go_wishlist('<?=$formcount-1?>')" style="max-width: 100%; height: auto; cursor:pointer;"> </div>
																	<!--<button type="button" class="basic_button wish" onClick="go_wishlist('<?=$formcount-1?>')"><span >찜하기</span></button>-->
																<?
																	} else {
																?>
																	<div><img src = "/m/skin/basic/img/1.png" onClick="check_login()" style="max-width: 100%; height: auto; cursor:pointer;"> </div>
...
?>
					<div class="main_prlist_list">
						<div class="wrapper">
							<div >
								<h1 onclick="location.href='productseleted.php'"><img src = "/m/images/skin/default/select_product.png" style="max-width: 100%; height: auto; cursor:pointer;"></h1>	
							</div>
							<? if(!empty($origPRList)){ ?>
								<div class="product_a" <?=($mainPRRow[gallery_type]=='B'?"style='margin:0px -15px;'":"")?>>
									<ul class="product_list" <?=($mainPRRow[gallery_type]=='B'?"style='display:flex;padding-left:10px;overflow-x:scroll;-webkit-overflow-scrolling:touch;'":"")?>>
...
?>
					<div class="main_prlist_list">
						<div class="wrapper">
							<div >
								<h1 onclick="location.href='productstr.php'"><img src = "/m/images/skin/default/favorate_store.png" style="max-width: 100%; height: auto; cursor:pointer;"></h1>		
							</div>
							<? if(!empty($origPRList)){ ?>
								<div class="product_a" <?=($mainPRRow[gallery_type]=='B'?"style='margin:0px -15px;'":"")?>>
									<ul class="product_list" <?=($mainPRRow[gallery_type]=='B'?"style='display:flex;padding-left:10px;overflow-x:scroll;-webkit-overflow-scrolling:touch;'":"")?>>
