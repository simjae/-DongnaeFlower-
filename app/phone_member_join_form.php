<INPUT type="hidden" name="id" value="<?=trim(str_replace("-","",$_POST["phoneNum"]))?>" />
<INPUT type="hidden" name="mobile" value="<?=trim(str_replace("-","",$_POST["phoneNum"]))?>"/>
...
-->
<div style="margin:10% 0 20px 0;font-weight:100;text-align:center;margin-top:50px;font-size:14px;line-height:26px">
	동네꽃집 회원가입을 통해
	<br>원활한 서비스를 이용 하실 수 있습니다.
</div>
<h1 style="margin:20px 0 0 0;font-weight:normal;text-align:center">동네꽃집 <span style="font-weight: 200">가입을 도와드릴게요.</span></h1>
<table border="0" cellpadding="0" cellspacing="0" width="90%" class="memJoinForm" style="margin:auto">
	<? if (!$loginType) { ?>
	<tr>
		<td style="overflow:hidden;">
			<INPUT type="text" name="id_str" value="<?=trim($_POST["phoneNum"])?>" maxLength="12" placeholder="전화번호" class="input" style="border:none;font-size: 18px;letter-spacing: 2px;font-weight: 500;text-align:center;" readonly/>
			<div style="font-weight:100;text-align:center;font-size:12px;line-height:14px">
				전화번호를 이용해서 로그인을 하실 수 있습니다.
			</div>
		</td>
	</tr>
	<tr>
		<td style="text-align:center">
			<label><INPUT type="radio" name="mem_type" value="1" class="formCheckbox" checked/> 일반회원</label>
			<span style="padding:0px 30px"></span>
			<label><INPUT type="radio" name="mem_type" value="2" class="formCheckbox" /> 법인회원</label>
		</td>
	</tr>
	<tr>
		<td style="overflow:hidden;">
	...
	<tr>
		<td>
			<div style="width:50%;float:left;">
				<INPUT type="text" name="name" value="<?=$name?>" maxLength="15" class="input" placeholder="이름" />
			</div>
			<div style="width:50%;float:left;text-align: center;line-height: 40px;">
				<label><INPUT type="radio" name="gender" value="1" class="formCheckbox" checked/> 남성</label>
...
?>
	<tr>
		<td style="border-bottom:solid 1px #cacac9;font-size:16px;padding-top:20px;">
		약관동의
		</td>
	</tr>
	<tr>
		<td style="padding-top:10px;">
		<INPUT type="checkbox" name="allChk" id="allChk" class="formCheckbox" value="Y" onclick="allCheck();" /> 전체동의
		</td>
	</tr>
	<tr>
		<td>
			<ul class="joinCheckList" style="margin-left:20px;margin-top:5px;">
				...
				<li class="receiveMail">
					<label><INPUT type="checkbox" name="news_mail_yn" class="formCheckbox" value="Y" <?=(($news_mail_yn=="Y") ? "checked" : "")?> /> 이메일 수신동의<span class="point7_b">선택</span></label>
				</li>
				<li class="receiveSms"><label><INPUT type="checkbox" name="news_sms_yn" class="formCheckbox" value="Y" <?=(($news_sms_yn=="Y") ? "checked" : "")?> /> SMS/알람 수신동의<span class="point7_b">선택</span></label>
				</li>
			</ul>
		</td>
	</tr>
