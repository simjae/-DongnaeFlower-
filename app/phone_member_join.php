...
$authSeq		= ($_GET['authSeq']) ? $_GET['authSeq'] : $_POST['authSeq'];
...
<div id="content">
	...
	<form name="form1" action="<?=$_SERVER[PHP_SELF]?>" method="post">
		...
		<? } ?>
		<input type="hidden" name="authSeq" value="<?=$authSeq?>" />
		...
		<? } ?>
		<? include ("phone_member_join_form.php"); ?>
	</form>
</div>
...
<script type="text/javascript">
	...
	<? if (!$loginType) { /* SNS에서 넘어 오지 않았을 때 */ ?>
		...
		if(form.id.value.length<4 || form.id.value.length>30) {
			alert("이메일는 4자 이상 30자 이하로 등록가능 합니다."); return;
		}
