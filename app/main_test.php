...
.notiBox .notiGruop .notiTextWrap{margin-left:20px;}
	...
	<div id="main_visual">
		...
		<div class="main01">
			...
			<div class="textBox"style="float: none;margin:0 0 0 27px;">
				<div class="content01" style="margin:0; font-size: 1.8em;">
					<span><?=$_ShopInfo->memname?></span>님 안녕하세요?
				</div>
				<div class="content02" style="font-size: 1.2em;">
					딱 맞는 꽃을 골라보세요
				</div>
			</div>
		</div>
		...
		<div class="notiBox">
			<div class="notiGruop">
				<div class="notiImg"> 
					<div class="venderSwiper" style="box-sizing:border-box;overflow:hidden;">
						...
						<?
							$imgSql = "SELECT
											...
											IF(
												...
												DATE_FORMAT(
													DATE_ADD(
														DATE_FORMAT(
														CONCAT(SUBSTR(regdate,1,4),'-',SUBSTR(regdate,5,2),'-',SUBSTR(regdate,7,2)),
														'%Y-%m-%d'
														),
														INTERVAL 1 MONTH
													),
											...
											LEFT JOIN tblvenderinfo vin
											on vm.vender = vin.vender
											WHERE
												vm.vender IN
												(
													SELECT * FROM
							...
							while ($imgRows = mysql_fetch_object($imgResult)) {
								$cont =  $imgRows->cont; 
								$new_vender_flg = $imgRows->new_vender_flg;
								
								$strDiv = "";
								if($new_vender_flg == 0){
									$strDiv .= '<div class="vender_new" style="z-index:99; position: absolute;top:15px; left:5px;width:35px;height:15px;background-color:#fae263;border-radius:10px;text-align:center;vertical-align:middle;line-height:15xpx;">';
									$strDiv .= '<font style="font-size:0.5rem;color:#505050;font-weight:500;">신규</font>';
									$strDiv .= '</div>';
								}else{
									$strDiv = "";
								}
						?>	
							<div class="swiper-slide venderWrap">
								<?=$strDiv?>
								<div class="shopImage" style=" margin-top:12px; width:90px!important;height:90px!important;border-radius:10px;background:url('/data/shopimages/vender/<?=$cont?>') no-repeat;background-size:cover"></div>
							</div>
		...
		<div class="mainBox" onclick="talkRequestOpen();">
			<div class="content01" style="margin: 20px 30px;">
				<div class="textBox" style="position: relative;top: 8px;">
					<div class="title">
						<div style="float: left;">스페셜 오더</div> 
						<div>
							<img style="width: 10px; margin:5px 0 0 15px" src="/app/skin/basic/svg/main_arrow_gray.svg" alt="">
						</div>
					</div>
					<div class="explain">
						원하는 꽃을 문의하면
		...
		<div class="notiBox" style="background-color: #FFEBEB;display:none;">
			<div class="notiGruop">
				<div class="notiImg"> 
					<div class="venderSwiper" style="box-sizing:border-box;overflow:hidden;">	
						<div class="swiper-wrapper">
				<?
					$tsSql ="SELECT
								ts.pridx,
								pm.cont,
								ROUND( (((t_consumerprice - t_sellprice) / t_consumerprice) * 100) ) AS discount
							FROM
								todaysale ts
					...
					while ($tsRow = mysql_fetch_object ($tsResult)) { 
						if ($i < 5) {
							$tsImg = $tsRow -> cont;
							$discount = $tsRow->discount;
							$strDiv = "";
							$strDiv .= '<div class="vender_new" style="z-index:99; position: absolute;top:15px; left:5px;width:35px;height:15px;background-color:#db1963;border-radius:10px;text-align:center;vertical-align:middle;line-height:15xpx;">';
							$strDiv .= '<font style="font-size:0.5rem;color:#f4bcd4;font-weight:500;">-'.$discount.'%</font>';
							$strDiv .= '</div>';
					?>
							<div class="swiper-slide venderWrap">
							<?=$strDiv?>
								<div class="shopImage" style="margin-top:12px;width:90px!important;height:90px!important;border-radius:10px;background:url('/data/shopimages/product/<?=$tsImg?>') no-repeat;background-size:cover"></div>
							</div>
					<?
							$i++;
						}
				...
				<div class="notiTextWrap">
					<div class="notiCnt tsCntNum" style="color:#e61e6e;">
						<input type="hidden" id="tsCnt" value="<?=$tsCnt?>">
			...
			<div class="content01" style="margin: 20px 30px;">
				<div class="textBox" style="position: relative;top: 8px;">
					<div class="title">
						...
						<div>
							<img style="width: 10px; margin:5px 0 0 30px" src="/app/skin/basic/svg/main_arrow_gray.svg" alt="">
						</div>
					</div>
					<div class="explain">
						신선한 꽃을 근처 꽃집에서
			...
			<div class="content01" >
				<div class="textBox">
					<div class="title" style="display: flex;">
						<div>단골 주문</div>
						<div>
							<img style="width: 10px; margin:4px 0 0 30px" src="/app/skin/basic/svg/main_arrow_gray.svg" alt="">
						</div>
					</div>
					<div class="explain">
						자주 사는 꽃집에서
