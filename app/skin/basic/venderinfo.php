<?
	...
	$infoSQL="SELECT 
				...
				FROM 
					tblvenderinfo v
					LEFT JOIN tblvenderstore vs ON
					v.vender = vs.vender
				WHERE v.vender = ".$vidx." ";
	...
	if(false !== $infoRes = mysql_query($infoSQL,get_db_conn())){
		...
		if($infoNumRows > 0){
			...
			$badge = mysql_result($infoRes,0,19);
...
<style>
	...
	.memberWrap{display: flex;margin: 0 0 5px 5px;}
	...
	.memberImg{width: 100%;margin-bottom:50px; }
	.memberImg img{width: 100%;height:auto;border-radius: 10px;}
	
	.talkProfileIcon {width: 45px;height: 45px;background: url(/app/skin/basic/svg/talkProfileIcon.svg);float: left;}
	...
	.pickbanner{width: fit-content;font-weight: 600;font-size: 0.9em;color: #e9387a;margin: 5px 7px 10px 15px;padding: 5px 9px 5px 9px;border-radius: 15px;line-height: 0.9em;box-shadow: 1px 1px 4px #ffe0e3;background-color: #ffe0e3;float: left;}
</style>
<div class="shopIntro">
	<?if($pagetype!="include"){?>
		<div class="h_area2">
			<h2 style="margin-top: 5px;" >꽃집소개</h2>
...
?>
	...
	<div class="review">
		<div class="review_circle">
			<?
				...
				$badgeCount = 0;
				for($i = 0 ; $i < 8 ; $i++){
					if($badgeCountArr[$i] > 10 || substr($badge,$i,1)=="Y"){
			?>
						<div class="badge">
	...
	<div class="shop_main">
		...
		<div class="shopEdit">
			...
			<?if (!strlen($brand_description) > 0) {?>
			<div class="talkProfileIcon"></div>
			<img src="/app/skin/basic/svg/no_description_msg.svg" style="margin-left:10px;width:60%;height:50px;">
			<?}?>
		</div>
	...
	<div class="shopReview">
		...
		<div class="reviewCount">
			...
			<span class="count1" style="font-size:1.3em"><?=$avg_marks?></span>
			<span class="count2" style="font-size:1.3em">(<?=$marks_count?>)</span>
		</div>
	</div>
	<div class="member">
	<?
		...
		if ($reviewNum > 0) {
			while($reviewRow = mysql_fetch_object($reviewResult)){
				...
				$mb_profileImage = $reviewRow->profile_photo;
				if($mb_profileImage == ""){
					$mb_profileImage = "/images/no_img.gif";
				}
				else{
					$mb_profileImage = "/data/profilephoto/".$mb_profileImage;
				}
				$vd_profileImage = $reviewRow->com_image;
				if($vd_profileImage == ""){
					$vd_profileImage = "/images/no_img.gif";
				}
				else{
					$vd_profileImage = "/data/profilephoto/".$vd_profileImage;
				}
		...
		?>
			...
			<div class="memberImg" >
				...
				<?}?>
				<p style="margin-left:15px;color:#464646; font-size:1.2em;"><?=$reviewcontent[0]?></p>
				<?
				...
				<?if (strlen($reviewcontent[1]) > 0) {?>
				<div class="replyContent" style="margin-left:8px;margin-top:50px;">
					<div class="circle" style="float:left;">
						<img src="<?=$vd_profileImage?>" alt="icon">
					</div>
					
					<div class="replyBalloon" style="margin-left:65px;padding-top:10px;padding-bottom:5px;padding-left:10px;min-height:30px;">
						<p style="color:#464646; font-size:1.2em;"><?=$reviewcontent[1]?></p>
					</div>
				</div>
				<?}?>
			</div>
		...
		<img src="/app/skin/basic/svg/no_review_msg.svg" style="margin-left:10px;width:60%;height:50px;">
		<?
		}
		?>
	</div>
