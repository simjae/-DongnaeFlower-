...
?>
		...
		<section class="basic_btn_area">
            ...
            <a href="<?=$shopping_url?>" class="basic_button goShopping">계속쇼핑</a>
            <a href="#" class="basic_button grayBtn1" onClick="javascript:orderInfo()">주문하기</a>
            <?
            //네이버 API 함수 호출
            if (function_exists('Naver_API_Product_detail')) {
                Naver_API_Product_detail_mobile_cssjs(); //css+js 호출(상단에 한번만 호출)
                Naver_API_Product_detail_btn('mobileBasket'); //네이버페이 버튼 출력
            }
            //네이버 API 함수 호출
            ?>
		</section>
        ...
        <?
            //네이버 API 함수 호출
            if (function_exists('Naver_API_Product_detail')) {
                Naver_API_Product_detail_basket($productcode, $_ShopInfo->getMemid(), $_ShopInfo->getTempkey(), 'Mobile');
            }
            //네이버 API 함수 호출
        ?>
